const timeline = [
    ...
    },
    {
      ...
      labor: 'Frontend Developer Trainee',
      description:
        ...
        <div className="mx-auto grid max-w-2xl grid-cols-1 gap-8 overflow-hidden lg:mx-0 lg:max-w-none lg:grid-cols-4">
          {timeline.map((item, index) => (
            <div key={index}>
              ...
              <p className="mt-6 text-lg/8 font-semibold tracking-tight text-green-900 dark:text-blue-300">{item.company}</p>
              <p className="mt-0 text-lg/8 font-semibold tracking-tight text-green-900 dark:text-gray-300">{item.labor}</p>
              <p className="mt-0 text-base/7 text-gray-600 dark:text-gray-300">{item.description}</p>
            </div>
          ))}
        </div>
      </div>
