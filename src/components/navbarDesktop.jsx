    ...
    <>    
      {
        isVisible ? (
            <nav aria-label="Global" className="font-mono animate-fade-down animate-duration-[1500ms] animate-delay-200 mx-auto flex max-w-7xl items-center justify-end lg:justify-center p-6 lg:px-8">
              ...
              <div className="hidden lg:flex lg:gap-x-12 bg-amber-100 bg-white/20 backdrop-blur-xl bg-opacity-80 rounded-2xl px-6 py-2 fixed top-8">
                  { 
                  navigationItems.map(( {name, to} , index) => (
                      <Link key={index} to={to} className="text-base font-semibold text-green-900 dark:text-gray-100 hover:underline">
                          {name}
                      </Link>
                      ))
                  }
